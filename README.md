...
- **`AUTH_COOKIE_DOMAIN`**: in production, `web` and `admin` are meant to run as subdomains of one apex domain so the session cookie is shared (`crossSubDomainCookies`) — set this if you want a single sign-on between them. Leave unset for local dev (different ports can't share a cookie domain anyway).
