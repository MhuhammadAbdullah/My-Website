...
- **About/Team**: `TeamMember`, `CoreValue`, `TimelineEvent`, `Certification`.
...
#### Team — `/api/v1/team`
Custom (JSON `socials`): `GET /`, `GET /admin` (`team:view`), `GET /:id`, `POST /` (`team:create`), `PATCH /:id` (`team:update` — partial `socials` updates never wipe existing links unless the key is explicitly present), `DELETE /:id` (`team:delete`).
