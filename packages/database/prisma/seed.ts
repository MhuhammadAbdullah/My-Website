  ...
  for (const [i, p] of projectSeed.entries()) {
    ...
    const project = await prisma.project.upsert({
      ...
      create: {
        ...
        client: p.client,
        summary: p.summary,
        overview: `${p.client} came to ${AGENCY_NAME} needing a partner who could move fast without breaking the parts of the product that already worked.`,
        problem: `${p.client}'s existing platform was slowing the team down — every new feature took weeks longer than it should have.`,
        research: "We ran stakeholder interviews and a technical audit of the existing stack before proposing any solution.",
        strategy: "We prioritized a phased rebuild: ship the highest-impact screens first, migrate data incrementally, keep the lights on throughout.",
        planning: "A 3-sprint roadmap broke the rebuild into shippable milestones with a demo at the end of every sprint.",
        wireframesNote: "Low-fidelity flows were validated with the client's internal team before any visual design began.",
        designNotes: "A dedicated design system was built in Figma so the client's team could extend the product post-launch.",
        developmentNotes: `Built on ${p.techs.join(", ")} with full type safety from database to UI.`,
        challenges: "Migrating live production data without downtime while three other teams depended on the existing system.",
        solutions: "A dual-write migration strategy let us cut over with zero downtime and a same-day rollback path.",
        results: p.results,
